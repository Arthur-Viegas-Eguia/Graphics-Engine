...
void meshRender(
	...
	if(mesh->attrDim == sha->attrDim){
		for(int i = 0; i < mesh->triNum; i++){
			...
			shadeVertex(sha->unifDim, unif, sha->attrDim, a, sha->varyDim, varyA);
			shadeVertex(sha->unifDim, unif, sha->attrDim, b, sha->varyDim, varyB);
			shadeVertex(sha->unifDim, unif, sha->attrDim, c, sha->varyDim, varyC);
