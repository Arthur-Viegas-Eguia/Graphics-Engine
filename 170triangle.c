 ...
 * the interpolated attributes of the triangle at a specific pixel.
 * Draws that point with the calculated color  by the fragment
 * shader on the screen.
*/
void renderPixel(int i, int j, const shaShading *sha, const double unif[], const texTexture *tex[], const double a[], const double m[2][2], const double betaMinusAlpha[], const double gammaMinusAlpha[]){
    const double x[2] = {i, j};
    double xMinusA[2], pAndQ[2], scaledP[sha->attrDim], scaledQ[sha->attrDim], scaledSum[sha->attrDim], chi[sha->attrDim], rgb[3];
    vecSubtract(2, x, a, xMinusA);
    mat221Multiply(m, xMinusA, pAndQ);
    vecScale(sha->attrDim, pAndQ[0], betaMinusAlpha, scaledP);
    vecScale(sha->attrDim, pAndQ[1], gammaMinusAlpha, scaledQ);
    vecAdd(sha->attrDim, scaledP, scaledQ, scaledSum);
    vecAdd(sha->attrDim, scaledSum, a, chi);
 ...
 * attributes of a triangle. These calculations are used
 * for every single point in it.
 * Renders the triangle on the screen with the color calculated
 * by the fragment shader.
*/
void triRenderHelper(const shaShading *sha, const double unif[], const texTexture *tex[], 
        const double a[], const double b[], const double c[]){
        int i, j;

        /*Declares the basic variables to calculate
        the interpolated color of a triangle*/
        double column1[2], column2[2], mat[2][2], m[2][2], betaMinusAlpha[sha->attrDim], gammaMinusAlpha[sha->attrDim];
        ...
        mat22Invert(mat, m);
        vecSubtract(sha->attrDim, b, a, betaMinusAlpha);
        vecSubtract(sha->attrDim, c, a, gammaMinusAlpha);
            ...
            if(!(a[0] == b[0] || a[0] == c[0])){
                ...
                for(i = ceil(a[0]); i <= floor(c[0]); i++){
                    for(j = ceil((a[1] +(((b[1] - a[1])/(b[0] - a[0])) * (i - a[0])))); j <= floor((a[1]+(((c[1] - a[1])/(c[0] - a[0])) * (i - a[0])))); j ++){
                        renderPixel(i, j, sha, unif, tex, a, m, betaMinusAlpha, gammaMinusAlpha);
                    }
                }
            }
...
*/
void triRender(const shaShading *sha, const double unif[], const texTexture *tex[], 
        const double a[], const double b[], const double c[]) {
    if(a[0] <= b[0] && a[0] <= c[0]){
