...
void meshRender(
	...
	double viewportA[4], viewportB[4], viewportC[4], tempA[4], tempB[4], tempC[4]; 
	if(mesh->attrDim == sha->attrDim){
		for(int i = 0; i < mesh->triNum; i++){
			...
			vecScale(4,1/viewportA[3], viewportA, tempA);
			vecScale(4,1/viewportB[3], viewportB, tempB);
			vecScale(4,1/viewportC[3], viewportC, tempC);
			vecCopy(4, tempA, varyA);
			vecCopy(4, tempB, varyB);
			vecCopy(4, tempC, varyC);
