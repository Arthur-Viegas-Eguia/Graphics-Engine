 ...
 * after that it creates an interpolated color
 * based on the rgb values for each vertex
 * and modulates the image by it, finally it
 * modulates ther esulting image by the
 * uniform attribute. It saves the color on
 * rgb
 */
void shadeFragment(
		...
		vecModulate(3, &unif[UNIFR], texColor, texUnif);
		vecModulate(3, &attr[ATTRR], texUnif, rgb);
}
